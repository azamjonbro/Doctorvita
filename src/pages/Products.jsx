...
function ProductCard({ product }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        <div className="text-[10px] uppercase tracking-[0.25em] text-rose">
          {product.category}
        </div>
        <h3 className="font-serif text-xl mt-1 line-clamp-1 text-noir">
          {product.name}
        </h3>
        <div className="mt-3">
          {hasDiscount ? (
            <div className="flex items-end justify-between gap-2">
              <div className="flex flex-col">
                <span className="text-xs original-price">
                  {price.toLocaleString()} so'm
                </span>
                <span className="font-bold text-lg price-final">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs uppercase tracking-wider text-stone group-hover:text-rose transition-colors">
                Ko'rish →
              </span>
            </div>
          )}
...
export default function Products() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "price-asc")
      list.sort(
        (a, b) => (a.final_price ?? a.price) - (b.final_price ?? b.price),
      );
    else if (sort === "price-desc")
      list.sort(
        (a, b) => (b.final_price ?? b.price) - (a.final_price ?? a.price),
      );
    ...
    <div className="bg-ivory min-h-screen" data-testid="products-page">
      ...
      <div className="bg-white border-b border-line">
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
          ...
          <h1 className="font-serif text-4xl md:text-5xl text-noir mt-2">
            Barcha mahsulotlar
          </h1>
          <p className="text-stone mt-2">
            Sizning go'zalligingiz uchun maxsus tanlov — {products.length} ta
            mahsulot
          </p>
        </div>
      </div>
        ...
        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="rounded-2xl bg-cream animate-pulse h-80"
              />
            ))}
          </div>
        ) : filtered.length === 0 ? (
