...
export default function AdminDashboard() {
    ...
    const startEdit = (p) => {
        ...
        setForm({
            ...
            sku: p.sku || "", units_per_package: Number(p.units_per_package) || 0,
        });
        setOpen(true);
    };
        ...
        <div className="bg-ivory min-h-screen" data-testid="admin-dashboard">
            ...
            <main className="max-w-7xl mx-auto px-6 md:px-10 py-8 space-y-6">
                <Tabs defaultValue="products">
                    <TabsList className="bg-cream">
                        ...
                        <TabsTrigger value="settings" data-testid="a-tab-settings">Sozlamalar</TabsTrigger>
                    </TabsList>
            ...
            <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setScanOrigin(null); }}>
                <DialogContent className="bg-ivory border-line rounded-2xl max-w-2xl max-h-[92vh] overflow-y-auto p-6 md:p-8">
                    ...
                    <form onSubmit={submitProduct} className="space-y-5" data-testid="product-form">
                        {scanOrigin && (
                        ...
                        {/* ---- Asosiy ---- */}
                        <FormSection title="Asosiy ma'lumotlar" icon={<Package className="w-3.5 h-3.5"/>}>
                            <Field label="Mahsulot nomi" full>
                                <Input data-testid="p-name" className={FIELD_INPUT} value={form.name} onChange={(e)=>setForm({...form, name:e.target.value})} required/>
                            </Field>
                            <Field label="Tavsif" full>
                                <Textarea data-testid="p-desc" rows={3} className="bg-white rounded-xl resize-none" value={form.description} onChange={(e)=>setForm({...form, description:e.target.value})} required/>
                            </Field>
                            <Field label="Kategoriya">
                                <Input data-testid="p-cat" className={FIELD_INPUT} value={form.category} onChange={(e)=>setForm({...form, category:e.target.value})}/>
                            </Field>
                            <Field label="Ombordagi qoldiq (dona)">
                                <Input data-testid="p-stock" type="number" min={0} className={FIELD_INPUT} value={form.stock} onChange={(e)=>setForm({...form, stock:e.target.value})}/>
                            </Field>
                        </FormSection>

                        {/* ---- Narxlar ---- */}
                        <FormSection title="Narxlar va chegirma" icon={<DollarSign className="w-3.5 h-3.5"/>} tinted>
                            <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <Field label={<>Xarid narxi (so'm) <span className="text-rose normal-case tracking-normal">· ichki</span></>}>
                                    <Input data-testid="p-cost" type="number" min={0} step="any" className={FIELD_INPUT} value={form.cost_price} onChange={(e)=>setForm({...form, cost_price:e.target.value})} required/>
                                </Field>
                                <Field label="Sotuv narxi (so'm)">
                                    <Input data-testid="p-price" type="number" min={0} step="any" className={FIELD_INPUT} value={form.price} onChange={(e)=>setForm({...form, price:e.target.value})} required/>
                                </Field>
                                <Field label="Chegirma (%)">
                                    <div className="relative">
                                        <Input data-testid="p-discount" type="number" min={0} max={100} step="any" className={`${FIELD_INPUT} pr-9`} value={form.discount_percent} onChange={(e)=>setForm({...form, discount_percent:e.target.value})}/>
                        ...
                        {/* ---- Identifikatsiya ---- */}
                        <FormSection title="Kod, qadoq va muddat" icon={<ScanLine className="w-3.5 h-3.5"/>}>
                            <Field label="Shtrix-kod / QR">
                                <Input data-testid="p-barcode" className={FIELD_INPUT} value={form.barcode} onChange={(e)=>setForm({...form, barcode:e.target.value})} placeholder="Ixtiyoriy"/>
                            </Field>
                            <Field label="SKU (artikul)">
                                <Input data-testid="p-sku" className={FIELD_INPUT} value={form.sku} onChange={(e)=>setForm({...form, sku:e.target.value})} placeholder="Ixtiyoriy"/>
                            </Field>
                            <Field label="Qadoqda dona / tabletka" hint="0 = nomdan (№120) avtomatik">
                                <Input data-testid="p-upp" type="number" min={0} className={FIELD_INPUT} value={form.units_per_package} onChange={(e)=>setForm({...form, units_per_package:e.target.value})}/>
                            </Field>
                            <Field label="Yaroqlilik muddati">
                                <Input data-testid="p-expiry" type="date" className={FIELD_INPUT} value={form.expiry_date} onChange={(e)=>setForm({...form, expiry_date:e.target.value})}/>
                            </Field>
                        </FormSection>

                        {/* ---- Rasm ---- */}
                        <FormSection title="Rasm" icon={<ImageIcon className="w-3.5 h-3.5"/>}>
                            <div className="md:col-span-2">
                                <ImageUploadField
                        ...
                        <Button type="submit" className="w-full bg-noir text-ivory hover:bg-rose rounded-full h-12 text-base mt-2" data-testid="p-submit">
                            {scanOrigin
...
function Field({ label, hint, full = false, children }) {
    return (
        <div className={`flex flex-col gap-1.5 ${full ? "md:col-span-2" : ""}`}>
            <Label className="text-[11px] font-semibold uppercase tracking-[0.08em] text-stone">{label}</Label>
            {children}
...
function FormSection({ title, icon, tinted = false, children }) {
    return (
        <section className={`rounded-2xl border p-4 md:p-5 ${tinted ? "bg-cream border-line" : "bg-white border-line"}`}>
            <div className="text-[11px] uppercase tracking-[0.14em] text-rose font-semibold flex items-center gap-1.5 mb-4">{icon}{title}</div>
    ...
    return (
        <div className="space-y-3">
            ...
            {mode === "file" ? (
                ...
                <Input data-testid="p-img" className={FIELD_INPUT} value={manualUrl} onChange={(e) => handleUrlChange(e.target.value)} placeholder="https://..."/>
            )}
