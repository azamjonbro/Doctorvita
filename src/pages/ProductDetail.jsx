...
export default function ProductDetail() {
  ...
  const pricing = useMemo(() => {
    ...
    return {
      ...
      final,
      amount,
      hasDiscount: discount > 0 && amount > 0,
    };
  }, [product]);
  ...
  const startOrder = () => {
    ...
    if (user && user.role === "customer") setStep("location");
      ...
      toast.error("Buyurtma berish uchun mijoz akkauntingiz bo'lishi kerak.");
      return;
    } else setStep("auth");
    setOpen(true);
  };
  ...
  const captureLocation = () => {
    if (!navigator.geolocation)
      return setErr("Brauzeringiz geolokatsiyani qo'llab-quvvatlamaydi.");
    setLoadingLoc(true);
    navigator.geolocation.getCurrentPosition(
      ...
      (e) => {
        setLoadingLoc(false);
        setErr("Joylashuvni olib bo'lmadi: " + e.message);
      },
  ...
  return (
    <div className="bg-ivory min-h-screen" data-testid="product-detail">
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-10">
        <button
          onClick={() => nav(-1)}
          className="text-sm uppercase tracking-widest text-stone hover:text-noir flex items-center gap-2 transition-colors"
        ...
        <div className="grid lg:grid-cols-2 gap-12 mt-8 items-start">
          ...
          <div className="stagger">
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.3em] text-rose bg-rose/10 px-3 py-1.5 rounded-full">
                {product.category}
              </span>
            ...
            <h1 className="font-serif text-4xl md:text-5xl text-noir mt-3">
              {product.name}
            </h1>
              ...
              {pricing.hasDiscount ? (
                <div className="space-y-2">
                  <div className="flex items-baseline gap-3 flex-wrap">
                    <span className="font-serif text-4xl price-final">
                      {pricing.final.toLocaleString()} so'm
                    </span>
                    <span className="text-xl original-price text-stone">
                      {pricing.price.toLocaleString()} so'm
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 bg-rose text-ivory text-sm px-3 py-1 rounded-full font-semibold">
                      <Percent className="w-3.5 h-3.5" /> {pricing.discount}%
                      chegirma
                    </span>
                ...
                <div className="font-serif text-4xl text-noir">
                  {pricing.price.toLocaleString()} so'm
                </div>
              )}
            </div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <Button
                onClick={startOrder}
                className="bg-noir text-ivory hover:bg-rose rounded-full h-12 px-8 btn-magnetic btn-shine"
              ...
              <Button
                variant="outline"
                className="border-noir text-noir rounded-full h-12 px-8 hover:bg-noir hover:text-ivory"
                onClick={() => nav("/contact")}
                data-testid="ask-btn"
              >
                Savolingiz bormi?
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 max-w-md text-sm">
              ...
              <div>
                ...
                <div className="text-noir mt-1">Toshkent bo'ylab</div>
              </div>
              {product.expiry_date && (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          className="bg-ivory border-line max-w-md rounded-2xl"
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription className="text-stone text-sm">
              {step === "auth" &&
                "Buyurtma berish uchun email orqali tezkor ro'yxatdan o'tish."}
              {step === "location" &&
                "Yetkazib berish uchun aniq lokatsiyangizni belgilaymiz."}
          ...
          {err && (
            <div
              className="text-sm text-rose bg-rose/10 rounded-md p-3"
          ...
          {step === "auth" && (
            <form
              ...
              className="space-y-3"
              data-testid="order-register-form"
            >
              ...
              <Button
                type="submit"
                className="w-full bg-noir text-ivory hover:bg-rose rounded-full h-11 mt-2"
          ...
          {step === "location" && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-stone">
                Joylashuvingiz faqat buyurtmani yetkazib berish uchun
                ishlatiladi.
              </p>
            </div>
          )}
          ...
          {step === "confirm" && (
            <div className="space-y-4" data-testid="order-confirm">
              <div className="rounded-xl bg-cream border border-line p-4 text-sm">
                <div className="text-stone uppercase tracking-widest text-[11px]">
                  Mahsulot
                </div>
                <div className="text-noir font-serif text-lg">
                  {product.name}
                </div>
                {pricing.hasDiscount ? (
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="text-xs bg-rose text-ivory rounded-full px-2 py-0.5">
                      −{pricing.discount}%
                    </span>
                  </div>
                ) : (
              ...
              <div className="rounded-xl bg-cream border border-line p-4 text-sm">
                ...
                <div className="text-noir font-mono">
