...
export default function ProductSearch({ onPick, onOpenCamera, inputRef }) {
    ...
    useEffect(() => {
        ...
        timer.current = setTimeout(async () => {
            ...
            setLoading(true);
            try {
                const { data } = await api.get("/pos/products/search", { params: { q: term, limit: 12 } });
    ...
    const onKey = (e) => {
        if (e.key === "ArrowDown") { e.preventDefault(); setOpen(true); setActive(a => Math.min(a + 1, results.length - 1)); }
            ...
            const exact = results.find(r => r.barcode === term || (r.sku && r.sku === term));
        ...
        <div ref={boxRef} className="relative" data-testid="pos-product-search">
            <div className="flex gap-2">
                <div className="relative flex-1">
                    ...
                    />
                    {loading && <Loader2 className="w-4 h-4 absolute right-3 top-1/2 -translate-y-1/2 text-stone animate-spin"/>}
                </div>
                <Button type="button" variant="outline" onClick={onOpenCamera}
                        className="h-11 border-rose text-rose hover:bg-rose hover:text-ivory rounded-xl px-3" title="Kamera skaneri">
                    <Camera className="w-4 h-4"/>
                </Button>
            </div>

            {open && (
                <div className="absolute z-40 mt-1 w-full bg-white border border-line rounded-xl shadow-lg overflow-hidden max-h-[380px] overflow-y-auto" data-testid="pos-search-results">
                    {results.length === 0 && !loading && (
                        <div className="p-4 text-sm text-stone text-center">Mahsulot topilmadi</div>
                    )}
                    {results.map((p, i) => {
                        ...
                        return (
                            ...
                            >
                                {p.image_url
                                    ? <img src={p.image_url} alt="" className="w-10 h-10 rounded-lg object-cover bg-cream flex-shrink-0"/>
                                    : <div className="w-10 h-10 rounded-lg bg-cream flex items-center justify-center"><Package className="w-4 h-4 text-stone"/></div>}
                                <div className="flex-1 min-w-0">
                                    <div className="text-sm text-noir font-medium line-clamp-1">{p.name}</div>
                                    <div className="text-[11px] text-stone flex gap-2 flex-wrap">
                                        {p.sku && <span>SKU: {p.sku}</span>}
                                        {p.barcode && <span>Kod: {p.barcode}</span>}
                                        {p.units_per_package > 1 && <span>· {p.units_per_package} dona/qadoq</span>}
                                    </div>
                                </div>
                                <div className="text-right flex-shrink-0">
                                    <div className="text-sm font-semibold text-noir">{fmtMoney(p.final_price ?? p.price)}</div>
                                    <div className={`text-[11px] ${out ? "text-red-600 font-semibold" : stock <= 5 ? "text-amber-700" : "text-stone"}`}>
