...
export default function ScannerPanel({
      ...
      <div className="space-y-4 animate-fade-in">
        {allScanned.length > 1 && (
          <div className="text-sm font-semibold text-noir bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
            🔍 Bu kodda {allScanned.length} ta mahsulot topildi — birini
            tanlang:
          </div>
        )}
        {allScanned.map((sp) => {
          ...
          return (
            <div
              key={sp.id}
              className={`bg-white rounded-2xl overflow-hidden ${sp.parent_barcode ? "border-2 border-amber-300" : "border border-line"}`}
            >
              {sp.parent_barcode && (
              ...
              <div className="flex items-center gap-3 p-3">
                <img
                  src={safeImageUrl(sp.image_url)}
                  alt={sp.name}
                  onError={(e) => {
                    e.currentTarget.onerror = null;
                    e.currentTarget.src = safeImageUrl(null);
                  }}
                  className="w-16 h-16 object-cover rounded-xl bg-cream flex-shrink-0"
                />
                <div className="flex-1 min-w-0 space-y-0.5">
                  <h3 className="font-serif text-base text-noir line-clamp-1">
                    {sp.name}
                  </h3>
                  ...
                  <div
                    className={`text-xs font-semibold ${outOfStock ? "text-red-600" : "text-stone"}`}
                  >
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <div className="text-sm text-rose bg-rose/10 rounded-md p-3">
          {error}
        </div>
      )}
      {!supported && (
        <div className="text-xs text-stone bg-cream rounded-md p-3">
          Brauzeringiz avtomatik skaner (BarcodeDetector) ni
          qo'llab-quvvatlamaydi. Kodni qo'lda kiriting.
        </div>
      )}
      <div className="flex gap-2">
        {!camActive ? (
          <Button
            onClick={startCamera}
            className="bg-rose text-ivory hover:bg-noir rounded-full flex-1"
          >
